...
def run(task, model, provider, workspace, max_iter, config, verbose, confirm, no_guardrails, no_nemoclaw, no_skills, cache, stream, list_tools, session, reasoning, plan, show_thought, mdconfig_dir):
    ...
    from code_agent.mdconfig.loader import MarkdownConfigLoader
...
def check():
    ...
    try:
        ...
        if count == 0:
            click.echo("  Run: python -c \"from code_agent.skills.seed import seed_library; seed_library()\"")
...
def model_list(provider, capability):
    ...
    from code_agent.serving.registry import ModelRegistry, ModelCapability
...
def model_register(model_id, provider, capabilities, context_window, max_output, cost_in, cost_out, alias):
    ...
    from code_agent.serving.registry import ModelRegistry
    registry = ModelRegistry()
...
def model_route(prompt, strategy, capability):
    ...
    from code_agent.serving.registry import ModelRegistry, ModelCapability
    from code_agent.serving.router import ModelRouter, RouterRule, RouteTarget, RoutingStrategy
...
def mcp(config_path):
    ...
    import asyncio
    from code_agent.mcp.client import MCPClient
...
def benchmark(scenario, save):
    ...
    from code_agent.benchmark import Benchmark, BenchmarkTask
...
def repl():
    ...
    from code_agent.repl import run_repl
    run_repl()
...
def completions(shell_type, install):
    ...
    from code_agent.completions.shell import generate_completions, install_completions
    if install:
...
def monitor_dashboard():
    ...
    from code_agent.monitor import MonitorDashboard
...
def monitor_metrics(name, since):
    ...
    from code_agent.monitor import MetricsCollector
...
def monitor_alerts(history):
    ...
    from code_agent.monitor import AlertManager
    mgr = AlertManager()
    if history:
...
def monitor_alert_add(name, metric_name, condition, threshold, cooldown):
    ...
    from code_agent.monitor import AlertManager, AlertRule, AlertCondition
    mgr = AlertManager()
    rule = AlertRule(
        name=name,
...
def monitor_server(host, port):
    ...
    from code_agent.monitor import MonitorServer
...
def hooks(action, hook_type):
    ...
    from code_agent.githooks import install_hook, remove_hook
    if action == "install":
...
def profile_list():
    ...
    from code_agent.profiles.base import ProfileManager
    mgr = ProfileManager()
...
def profile_save(name, agent, llm, desc):
    ...
    from code_agent.profiles.base import Profile, ProfileManager
    import json
...
def notify(action, title, body, webhook, slack):
    ...
    import json
    from code_agent.notify.notifier import Notification
    ...
    if slack:
        ...
        from code_agent.notify.slack import WebhookNotifier
...
def schedule_add(name, task, interval, cron, profile, tags, max_retries, timeout, provider):
    ...
    from code_agent.scheduler.base import ScheduledTask, RetryPolicy
...
def trace_list(limit):
    ...
    from code_agent.trace.collector import TraceCollector
...
def export_sessions(fmt, output):
    """Export all sessions."""
    from code_agent.export.session_export import SessionExporter
...
def export_archive(output):
    ...
    from code_agent.export.full_export import FullExporter
...
def health(report):
    ...
    from code_agent.health.checker import HealthChecker
...
def optimize(prompt, auto):
    ...
    from code_agent.optimizer.optimizer import PromptOptimizer
    opt = PromptOptimizer()
...
def fallback(prompt, providers, max_attempts):
    ...
    from code_agent.fallback.chain import FallbackChain
    from code_agent.llm.base import LLMConfig
...
def template_list():
    ...
    from code_agent.templates.manager import TemplateManager
    mgr = TemplateManager()
    for name in mgr.list():
        t = mgr.get(name)
...
def quality(path, pattern, json_output):
    ...
    from code_agent.quality.reporter import QualityReporter
...
def errors(error_text, action, source):
    ...
    from code_agent.learner.learner import ErrorLearner
...
def context(action, content, tier, source, max_tokens, reserve, add_demo, detail, session_id):
    ...
    if action == "visual":
        from code_agent.context.display import render_cli_context
...
def smells(path, pattern, json_output):
    ...
    from code_agent.smells.detector import SmellDetector
...
def validate(config_path):
    ...
    from code_agent.validate.config import ConfigValidator
...
def licenses(path, json_output):
    ...
    from code_agent.licenses.scanner import LicenseScanner
...
def promptver_list():
    ...
    from code_agent.promptversion.manager import PromptVersionManager
...
def daemon(action, config_path, api_port, dashboard_port):
    ...
    from code_agent.runner.daemon import AgentDaemon, DaemonConfig, run_daemon
...
def abtest(task, model_a, model_b, runs, name):
    ...
    from code_agent.abtest.runner import ABTestRunner, ABTestConfig
...
def deps(report, path):
    ...
    from code_agent.depupdater.updater import DepUpdater
    updater = DepUpdater()
...
def sessearch(query, max_results):
    ...
    from code_agent.sessearch.searcher import SessionSearchEngine
...
def tenant_create(name, workspace):
    """Create a new tenant."""
    from code_agent.tenants.manager import TenantManager
    mgr = TenantManager()
...
def nlquery(query):
    ...
    from code_agent.nlquery.engine import NLQueryEngine
...
def codesearch(query, path, kind, find_defs, do_build):
    ...
    from code_agent.codesearch.base import CodeSearchEngine
...
def apidocs(path, title, version, output, fmt):
    ...
    from code_agent.apidocs.base import ApiDocGenerator
...
def dataviz(data_source, chart_type, title, x_key, y_key, output, width, height):
    ...
    from code_agent.dataviz.base import DataVizEngine, ChartType, ChartConfig
...
def profile(target, script, code, output):
    ...
    from code_agent.profilers.base import CodeProfiler
...
def loganalyze(file, fmt, level, search, show_summary):
    ...
    from code_agent.loganalyze.base import LogAnalyzer
...
def coverage(path, target, extra_args, output):
    ...
    from code_agent.coverage.base import CoverageAnalyzer
...
def boilerplate(template_name, list_tmpl, name, output, params):
    ...
    from code_agent.boilerplate.base import BoilerplateGenerator
...
def envmgr(action, name, python_path, packages, envs_dir):
    ...
    from code_agent.envmgr.base import EnvManager
...
def migrate(plan_name, list_plans, path, do_analyze, do_apply, common_fixes):
    ...
    from code_agent.migrate.base import CodeMigrator
...
def privacy(query, local_model, cloud_model, sensitive_model, output_json):
    ...
    from code_agent.privacy.router import PrivacyRouter
...
def openshell(action, target, profile_name, check_type):
    ...
    from code_agent.openshell.policy import OpenShellPolicy, Decision
    policy = OpenShellPolicy(profile_name)
    if action == "status":
        click.echo(policy.summary_text())
    elif action == "list":
        from code_agent.openshell.policy import OpenShellPolicy as OSP
...
def browser(url, search, do_extract, list_tabs, close):
    ...
    from code_agent.browser.engine import BrowserEngine
...
def channel(action, message, session_id, export_path):
    ...
    from code_agent.channels.manager import ChannelManager, ChannelType
    mgr = ChannelManager()
...
def voice(action, text, output):
    ...
    from code_agent.voice.engine import VoiceEngine
    engine = VoiceEngine()
...
def selflearn(action, category, store_path, pref_key):
    ...
    from code_agent.selflearn.engine import SelfLearningEngine
...
def integrate(integration, action, event, to_addr, subject, body):
    ...
    if integration == "status":
        from code_agent.integrations.email import EmailIntegration
        email = EmailIntegration()
        click.echo(f"Email configured: {email.is_configured()}")
    elif integration == "calendar":
        from code_agent.integrations.calendar import CalendarIntegration, CalendarEvent
        ...
        from code_agent.integrations.email import EmailIntegration, EmailMessage
        email = EmailIntegration()
        if action == "configure":
...
def remote(action, host, port, user, key_path, name, command, timeout):
    ...
    from code_agent.remote.agent import RemoteAgent, RemoteHost
    agent = RemoteAgent()
    if action == "list":
...
def backup(action, name, project, output, modules):
    ...
    from code_agent.backup.manager import BackupManager
...
def changelog(range_spec, max_count, fmt, output, path):
    ...
    from code_agent.changelog.generator import ChangelogGenerator
...
def mdconfig_init(project, force):
    ...
    from code_agent.mdconfig.generator import (
...
def mdconfig_show(file, dir_path, json_output):
    ...
    from code_agent.mdconfig.parser import parse_md, extract_frontmatter
...
def toolbuilder(name, description, behavior, output, do_save):
    ...
    from code_agent.toolbuilder.builder import ToolBuilder
...
def ws(action, port, host, message, event_type):
    ...
    from code_agent.ws.server import WebSocketServer
...
def desktop(geometry, fallback):
    ...
    try:
        ...
        if fallback:
            ...
            try:
                from code_agent.ui.server import create_ui_app
                from code_agent import AgentConfig
                import uvicorn
...
def skill_add(body, tags):
    ...
    from code_agent.skills.base import Skill, SkillLibrary
...
def skillv2_episode(instruction, difficulty, seed, provider, model):
    ...
    from code_agent.llm.base import LLM
    from code_agent.skills.v2 import SkillManagerV2
...
def skillv2_evaluate(skill_id):
    ...
    from code_agent.skills.v2 import SkillManagerV2
    from code_agent.skills.v2.evaluation import SkillEvaluator, EvalStore
...
def skillv2_benchmark():
    ...
    from code_agent.skills.v2 import SkillManagerV2
    from code_agent.skills.v2.evaluation import EvalStore
    ev = EvalStore()
    comp = ev.comparison()
    if not comp:
...
def skillv2_credit():
    ...
    from code_agent.skills.v2 import CreditStore
    cs = CreditStore()

...
(function () {
  ...
  let state = {
    ...
    buildForm: { name: '', vertical: 'healthcare', desc: '', tools: [] },
    buildTagInput: '',
  };
  ...
  function render() {
    ...
    return `
      <div class="page page--agents">
        <div class="page__inner">
          <div class="page-header">
            ...
            <div style="display:flex;gap:8px">
              ...
              <button class="btn btn--primary" data-action="open-build-modal">${icons.plus(13)} Build agent</button>
            </div>
          </div>
        ...
        <!-- BUG 8: Build Agent Modal -->
        ${state.buildModalOpen ? renderBuildModal() : ''}
      </div>
    `;
  }
  ...
  function renderBuildModal() {
    return `
      <div class="modal-overlay" data-close-modal style="position:fixed;inset:0;background:rgba(9,9,14,0.7);backdrop-filter:blur(6px);z-index:200;display:flex;align-items:center;justify-content:center">
        <div class="modal-card" style="background:var(--bg-2);border:1px solid var(--border);border-radius:var(--r-xl);width:480px;max-width:92vw;box-shadow:var(--shadow-lg);overflow:hidden" onclick="event.stopPropagation()">
          <div style="padding:24px 28px;border-bottom:1px solid var(--border-subtle);display:flex;align-items:center;justify-content:space-between">
            <div>
              <h3 style="font-size:17px">Build a new agent</h3>
              <div style="color:var(--text-2);font-size:12.5px;margin-top:3px">Define the agent's purpose, vertical, and toolbelt.</div>
            </div>
            <button class="icon-btn" data-close-modal-btn>${icons.x(16)}</button>
          </div>
          <div style="padding:24px 28px;display:flex;flex-direction:column;gap:16px">
            <div class="field">
              <label>Agent name</label>
              <input class="input" data-build-name placeholder="e.g. ClaimProcessor" value="${escapeHTML(state.buildForm.name)}" />
            </div>
            <div class="field">
              <label>Vertical</label>
              <select class="input" data-build-vertical style="height:36px;padding:0 12px">
                ${window.MOCK.verticals.map(v => `
                  <option value="${v.id}" ${state.buildForm.vertical === v.id ? 'selected' : ''}>${escapeHTML(v.name)}</option>
                `).join('')}
              </select>
            </div>
            <div class="field">
              <label>Description</label>
              <textarea class="input" data-build-desc placeholder="What does this agent do?" style="height:72px;padding:10px 12px;line-height:1.5">${escapeHTML(state.buildForm.desc)}</textarea>
            </div>
            <div class="field">
              <label>Tools <span style="color:var(--text-3);font-weight:400">(type + Enter to add)</span></label>
              <div style="display:flex;flex-wrap:wrap;gap:6px;padding:8px;background:var(--bg-2);border:1px solid var(--border-subtle);border-radius:var(--r-sm);min-height:42px;align-items:center" data-tools-container>
                ${state.buildForm.tools.map(t => `
                  <span class="composer__file-chip" data-remove-tool="${escapeHTML(t)}">
                    ${escapeHTML(t)}
                    <button style="margin-left:4px;color:var(--text-3);display:inline-flex;align-items:center" data-remove-tool-btn="${escapeHTML(t)}">${icons.x(10)}</button>
                  </span>`).join('')}
                <input data-build-tag-input placeholder="Add tool…" style="flex:1;min-width:80px;background:none;border:none;outline:none;color:var(--text);font-size:13px;padding:2px 4px" value="${escapeHTML(state.buildTagInput)}" />
              </div>
            </div>
          </div>
          <div style="padding:16px 28px;border-top:1px solid var(--border-subtle);display:flex;gap:8px;justify-content:flex-end">
            <button class="btn btn--ghost" data-close-modal-btn>Cancel</button>
            <button class="btn btn--primary" data-action="create-agent">${icons.plus(13)} Create agent</button>
          </div>
        </div>
      </div>`;
  }
  ...
  function openBuildModal() {
    ...
    state.buildForm = { name: '', vertical: 'healthcare', desc: '', tools: [] };
    state.buildTagInput = '';
    rerenderModal();
  }

  function closeBuildModal() {
    state.buildModalOpen = false;
  ...
  function rerenderModal() {
    ...
    if (!state.buildModalOpen) return;
    const div = document.createElement('div');
    div.innerHTML = renderBuildModal();
    document.querySelector('.page--agents').appendChild(div.firstElementChild);
    wireModal();
  }
  ...
  function wireModal() {
    ...
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay) closeBuildModal();
    });

    // Close buttons
    overlay.querySelectorAll('[data-close-modal-btn]').forEach(btn => {
      btn.addEventListener('click', closeBuildModal);
    });

    // Name input
    const nameInput = overlay.querySelector('[data-build-name]');
    if (nameInput) {
      nameInput.focus();
      nameInput.addEventListener('input', () => state.buildForm.name = nameInput.value);
    }

    // Vertical select
    const vertSelect = overlay.querySelector('[data-build-vertical]');
    if (vertSelect) vertSelect.addEventListener('change', () => state.buildForm.vertical = vertSelect.value);

    // Desc textarea
    const descInput = overlay.querySelector('[data-build-desc]');
    if (descInput) descInput.addEventListener('input', () => state.buildForm.desc = descInput.value);

    // Tag input — type + enter to add
    const tagInput = overlay.querySelector('[data-build-tag-input]');
    if (tagInput) {
      tagInput.addEventListener('input', () => state.buildTagInput = tagInput.value);
      tagInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && tagInput.value.trim()) {
          e.preventDefault();
          const tag = tagInput.value.trim();
          if (!state.buildForm.tools.includes(tag)) {
            state.buildForm.tools.push(tag);
          }
          state.buildTagInput = '';
          rerenderModal();
        }
    ...
    // Remove tool tags
    overlay.querySelectorAll('[data-remove-tool-btn]').forEach(btn => {
      btn.addEventListener('click', () => {
        const t = btn.dataset.removeToolBtn;
        state.buildForm.tools = state.buildForm.tools.filter(x => x !== t);
        rerenderModal();
      });
    });

    // Create button
    const createBtn = overlay.querySelector('[data-action="create-agent"]');
    if (createBtn) {
      createBtn.addEventListener('click', () => {
        const name = (overlay.querySelector('[data-build-name]')?.value || '').trim();
        const vertical = overlay.querySelector('[data-build-vertical]')?.value || 'healthcare';
        const desc = (overlay.querySelector('[data-build-desc]')?.value || '').trim();
        if (!name) {
          overlay.querySelector('[data-build-name]')?.focus();
          return;
        }
        // Add to mock data
        const v = window.MOCK.verticals.find(x => x.id === vertical);
        const newAgent = {
          id: 'agent-' + Date.now(),
          vertical,
          name,
          desc: desc || 'Custom agent created by user.',
          tools: state.buildForm.tools.length,
          status: 'online',
          icon: v?.icon || 'sparkles',
          color: v?.color || '#6E6EF5',
        };
        window.MOCK.agents.push(newAgent);
        closeBuildModal();
        updateGrid();
        window.Orchestra.toast('Agent created successfully', 'success');
      });
    }
  }
  ...
  function mount(root) {
    ...
    // BUG 8: Build agent button
    root.querySelector('[data-action="open-build-modal"]')?.addEventListener('click', openBuildModal);

"""
orchestra/briefing_tools.py
────────────────────────────
Agent-callable tools for managing daily briefings from inside Orchestra's
agent loop. These are wired into agent_loop.py's tool registry and are
gated to Enterprise-tier customers.

Tools exposed to Kimi K2.5:
...
class BriefingToolExecutor:
    """
    Handles execution of all briefing tool calls from the agent loop.
    Wired into agent_loop.py's tool dispatch table.
    """
    ...
    def __init__(self, scheduler, customer_tier_fn=None):
        ...
        Args:
            ...
            customer_tier_fn: callable(customer_id) -> str tier label
        """
    ...
    async def execute(self, tool_name: str, arguments: dict) -> str:
        ...
        try:
            ...
            return json.dumps({"error": str(exc), "upgrade_url": "horizon-orchestra.com/billing"})
...
# ──────────────────────────────────────────────
# Tool table entry for agent_loop.py integration
# ──────────────────────────────────────────────

def get_briefing_tools(scheduler, customer_tier_fn=None) -> tuple[list[dict], "BriefingToolExecutor"]:
    """
    Returns (tool_definitions, executor) ready to be merged into
    agent_loop.py's tool registry.

    Usage in agent_loop.py:
        from .briefing_tools import get_briefing_tools
        briefing_tools, briefing_executor = get_briefing_tools(scheduler)
        tools = base_tools + briefing_tools  # Enterprise customers only
    """

"""
orchestra/briefing_monitor.py
──────────────────────────────
News fetcher and briefing composer for Orchestra Enterprise daily briefings.
...
from .briefing_config import BriefingConfig, BriefingTopic
...
class NewsItem:
    ...
    def fingerprint(self) -> str:
        """Deduplication key based on URL."""
...
# ──────────────────────────────────────────────
# Search provider (Perplexity Sonar)
# ──────────────────────────────────────────────

class SonarSearchProvider:
    """
    Uses Perplexity Sonar Pro for real-time search with citations.
    Falls back to a direct HTTP search if Sonar key is unavailable.
    """
    ...
    async def search(self, query: str, max_results: int = 5) -> list[NewsItem]:
        """Run a single query against Sonar Pro and return NewsItems."""
        try:
            ...
            # Degrade gracefully — return empty list, log error
            return []
    ...
    def _parse_bullets(self, raw: str, citations: list[str],
        ...
        for line in lines:
            if not line.startswith(("-", "•", "*", "[")):
                continue
            # Extract source from [Source] prefix
            source_match = re.match(r"[-•*]?\s*\[([^\]]+)\]", line)
            source = source_match.group(1) if source_match else "Unknown"
            # Strip bullet/source prefix
            text = re.sub(r"^[-•*\s]*\[[^\]]+\]\s*", "", line)
            # Extract inline URL if present
...
class BriefingMonitor:
    """
    Core engine that fetches news, deduplicates, and composes
    the daily briefing email using Kimi K2.5 Thinking mode.
    """
    ...
    async def run(self, config: BriefingConfig) -> BriefingResult:
        """
        Full pipeline: fetch → deduplicate → compose → detect breaking.
        Returns a BriefingResult ready for delivery.
        """
        date_label = datetime.now(timezone.utc).strftime("%B %d, %Y")

        # 1. Fetch news for all topics in parallel
        topic_results = await self._fetch_all_topics(config.topics)

        # 2. Detect breaking news per topic
        for tr in topic_results:
            tr.is_breaking, tr.breaking_headline = self._detect_breaking(tr)

        # 3. Compose email body with Kimi K2.5
    ...
    async def _fetch_topic(self, topic: BriefingTopic) -> TopicResult:
        ...
        # Flatten and deduplicate by URL fingerprint
        seen: set[str] = set()
        items: list[NewsItem] = []
    ...
    def _detect_breaking(
        ...
        """
        Check if any news item triggers a breaking_keyword for its topic.
        Returns (is_breaking, headline_string).
        """
    ...
    async def _compose_email(
        ...
        """
        Use Kimi K2.5 Thinking mode to synthesize all topic results
        into a clean, well-structured plain-text briefing email.
        """
        # Build raw research context
        ...
        # Build section structure for Kimi to follow
        section_guide = ""
        if config.sections:
            ...
            # Auto-generate sections from topic names
            section_guide = "Create one section per topic using the topic name as header (ALL CAPS)."

        system_prompt = f"""You are Orchestra's Intelligence Briefing Engine.
...
{'-' * 60}
Delivered by Orchestra Enterprise | horizon-orchestra.com
"""
        ...
        response = await self.kimi.chat.completions.create(
            ...
            max_tokens=4096,
            # Thinking mode ON for better synthesis and citation accuracy
        )

    ...
    def save(self, tool: GeneratedTool) -> str:
        ...
        if not init_file.exists():
            init_file.write_text(
                f"from orchestra.code_agent.custom_tools.{Path(tool.path).stem} import {tool.name}\n",
                encoding="utf-8",
            )
        else:
            ...
            if tool.name not in content:
                init_file.write_text(
                    content + f"from orchestra.code_agent.custom_tools.{Path(tool.path).stem} import {tool.name}\n",
                    encoding="utf-8",
                )
        return tool.path

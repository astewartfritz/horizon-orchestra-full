...
def register_healthcare_routes(app: FastAPI) -> None:
    ...
    async def hc_list_appointments(date: str = "", patient_id: str = "", status: str = ""):
        appts = list_appointments(date=date, patient_id=patient_id, status=status)
        return {"appointments": [a.to_dict() for a in appts]}

    @app.post("/api/healthcare/appointments")
    async def hc_create_appointment(request: Request):
        data = await request.json()
        try:
            a = create_appointment(data)
        except KeyError as e:
            raise HTTPException(400, f"Missing required field: {e}")
        return a.to_dict()
    ...
    async def hc_list_encounters(patient_id: str = ""):
        encs = list_encounters(patient_id=patient_id)
        return {"encounters": [e.to_dict() for e in encs]}

    @app.post("/api/healthcare/encounters")
    async def hc_create_encounter(request: Request):
        data = await request.json()
        try:
            e = create_encounter(data)
    ...
    async def hc_list_claims(patient_id: str = "", status: str = ""):
        claims = list_claims(patient_id=patient_id, status=status)
        return {"claims": [c.to_dict() for c in claims]}

    @app.post("/api/healthcare/claims")
    async def hc_create_claim(request: Request):
        data = await request.json()
        try:
            c = create_claim(data)
        except KeyError as err:
    ...
    async def hc_claim_from_encounter(enc_id: str):
        ...
        c = create_claim(claim_data)
        # link claim back to encounter
